...
export const createLocationOfInterest = async (req: Request, res: Response) => {
  try {
    ...
    const locationOfInterest = await prisma.location_of_interest.create({
      data: newLocationOfInterest,
    });
...
export const getLocationOfInterestByPatrolId = async (
  req: Request,
  res: Response
) => {
  try {
    ...
    const locationsOfInterestToSend = locationsOfInterest.map(
